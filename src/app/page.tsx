...
export default function ProfilePage() {
  ...
  const [subscriptionDetails, setSubscriptionDetails] =
    useState<Subscription>(mockSubscriptionData);
  ...
  function mapDbSubscriptionToUi(row: any): Subscription {
    return {
      // DB id is UUID (string); your type is number — keeping cast to match your model.
      id: row.id as unknown as string,
      status: row.status as SubscriptionStatus,
      isTrialSubscription: false,
      cancelAtPeriodEnd: row.status === "pending_cancellation",
      currentPeriodEnd: new Date(
        Date.now() + 30 * 24 * 60 * 60 * 1000
      ).toISOString(),
      monthlyPrice: Math.round((row.monthly_price ?? 0) / 100),
      isUCStudent: false,
      hasManagedAccess: false,
      managedOrganization: null,
      downsellAccepted: false,
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!firstUser) {
        console.warn("No users found in DB");
        return;
      }

      // Step 2: set userDetails
      setUserDetails(firstUser);

      // Step 3: fetch subscription using that user’s id
      const fresh = await fetchSubscriptionByIdFromDb(firstUser.id);
      if (fresh) {
        console.log(
  ...
  useEffect(() => {
    if (!showCancelModal && userDetails?.id) {
      (async () => {
        const fresh = await fetchSubscriptionByIdFromDb(userDetails.id);
        if (fresh) {
          console.log(
            "Refetched subscription details:\n",
            JSON.stringify(fresh, null, 2)
          );
          setSubscriptionDetails(fresh);
        }
  ...
  async function fetchSubscriptionByIdFromDb(
    userId: string
  ): Promise<Subscription> {
    if (!userId) {
      console.warn("fetchSubscriptionByIdFromDb called with empty userId");
    }

    const { data, error } = await supabase.rpc("fn_get_subscription_by_id", {
      p_id: userId,
    });
    ...
    return mapDbSubscriptionToUi(rows[0]);
  }

  async function fetchUsersFromDb(): Promise<User[]> {
    const { data, error } = await supabase
      .from("get_v_users")
      .select("id, email") // created_at not needed for User
                ...
                <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                  ...
                  <div className="flex items-center space-x-2">
                    {subscriptionDetails.status === "active" &&
                      !subscriptionDetails.isTrialSubscription &&
