...
type Route =
  | 'question'
  | 'yes.step1' | 'yes.step2' | 'yes.step3' | 'yes.done'
  | 'no.step1'
  | 'no.usage'
  | 'no.step2'
  | 'no.step3'
  ...
  return (
    <div className="flex items-center gap-2 text-xs text-gray-500">
      {Array.from({ length: total }).map((_, i) => {
        const bar =
          i < active - 1 ? completedColor :
          i === active - 1 ? currentColor :
          inactiveColor;
        return <span key={i} className={`h-1.5 w-6 rounded-full ${bar}`} />;
      })}
      {showLabel && <span>Step {active} of {total}</span>}
    </div>
  );
}
...
function HeaderBar({
  ...
  mobileTitle,
  onBack, showBack, stepActive, stepTotal, onClose,
}: {
  title?: string;
  mobileTitle?: string;
  showBack?: boolean;
  ...
  return (
    <div className="relative flex items-center justify-between border-b px-4 sm:px-6 py-3">
      <div className="w-24">
        {showBack && (
          <button onClick={onBack} className="flex items-center gap-2 text-sm text-gray-700 hover:underline">
      ...
      <div className="flex items-center gap-4">
        ...
        {stepActive && stepTotal ? <Stepper active={stepActive} total={stepTotal} /> : null}
      </div>

      <button onClick={onClose} className="w-24 text-right rounded p-2 text-gray-500 hover:bg-gray-100" aria-label="Close">
        <svg width="20" height="20" viewBox="0 0 24 24" stroke="currentColor" fill="none" className="inline">
    ...
    <>
      ...
      <div className="p-4 sm:p-6 md:grid md:grid-cols-2 md:gap-6">
        {imageMode === 'top-mobile-right-desktop' && (
          <>
            <div className="order-1 md:order-2 overflow-hidden rounded-xl">{ImagePane}</div>
        ...
        {imageMode === 'right-hide-mobile' && (
          <>
            ...
            <div className="hidden md:block md:order-2 overflow-hidden rounded-xl">{ImagePane}</div>
          </>
        )}
        {imageMode === 'none' && <div className="md:col-span-2">{children}</div>}
      </div>
    </>
  );
}
...
type PillGroupProps<T extends string> = {
  value?: T; onChange: (v: T) => void; options: PillOption<T>[]; columns?: 2 | 3 | 4;
};
function PillGroup<T extends string>({ value, onChange, options, columns = 4 }: PillGroupProps<T>) {
  const col = columns === 2 ? 'grid-cols-2' : columns === 3 ? 'grid-cols-3' : 'grid-cols-4';
  function onKeyDown(e: React.KeyboardEvent) {
    const idx = Math.max(0, options.findIndex(o => o.value === value));
    if (e.key === 'ArrowRight' || e.key === 'ArrowDown') {
      onChange(options[(idx + 1) % options.length].value as T); e.preventDefault();
    } else if (e.key === 'ArrowLeft' || e.key === 'ArrowUp') {
      onChange(options[(idx - 1 + options.length) % options.length].value as T); e.preventDefault();
    }
  }
  ...
  const active =
    'bg-[#8952fc] text-white border-[#8952fc] shadow'; // selected: purple bg, white text

  const inactive =
    'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'; // unselected
  ...
  return (
    <div>
      <h2 className="text-[28px] sm:text-[30px] font-bold leading-snug text-gray-900">
        Hey mate,<br /> <span>Quick one before you go.</span>
      </h2>
...
function CancelYesStep1({
  cancellationData, patchCancellation,
  userEmail, foundViaMM, setFoundViaMM, appliedCount, setAppliedCount, emailedCount, setEmailedCount,
  interviewsCount, setInterviewsCount, onNext,
}: {
  cancellationData: CancellationRow; patchCancellation: (p: Partial<CancellationRow>) => void;
  userEmail?: string;
  foundViaMM?: 'yes' | 'no'; setFoundViaMM: (v: 'yes' | 'no') => void;
  appliedCount?: string; setAppliedCount: (v: string) => void;
  emailedCount?: string; setEmailedCount: (v: string) => void;
  interviewsCount?: string; setInterviewsCount: (v: string) => void;
  onNext: () => void;
}) {
  const allAnswered = Boolean(foundViaMM && appliedCount && emailedCount && interviewsCount);
  return (
    <div>
      <h2 className="mb-4 text-[26px] sm:text-[28px] font-bold text-gray-900">Congrats on the new role! 🎉</h2>

      <div className="mb-4">
        <label className="mb-1 block text-[11px] tracking-wide text-gray-600">Did you find this job with MigrateMate?*</label>
        <PillGroup value={foundViaMM} onChange={setFoundViaMM} options={[{ label: 'Yes', value: 'yes' }, { label: 'No', value: 'no' }]} columns={2} />
      </div>

      <div className="mb-4">
        <label className="mb-1 block text-[11px] tracking-wide text-gray-600">How many roles did you <span className="underline">apply</span> for through Migrate Mate?*</label>
        <PillGroup
          value={appliedCount} onChange={setAppliedCount} columns={4}
          options={[{ label: '0', value: '0' }, { label: '1 – 5', value: '1-5' }, { label: '6 – 20', value: '6-20' }, { label: '20+', value: '20+' }]}
        />
      </div>

      <div className="mb-4">
        <label className="mb-1 block text-[11px] tracking-wide text-gray-600">How many companies did you <span className="underline">email</span> directly?*</label>
        <PillGroup
          value={emailedCount} onChange={setEmailedCount} columns={4}
          options={[{ label: '0', value: '0' }, { label: '1–5', value: '1-5' }, { label: '6–20', value: '6-20' }, { label: '20+', value: '20+' }]}
        />
      </div>

      <div className="mb-5">
        <label className="mb-1 block text-[11px] tracking-wide text-gray-600">How many different companies did you <span className="underline">interview</span> with?*</label>
        <PillGroup
          value={interviewsCount} onChange={setInterviewsCount} columns={4}
          options={[{ label: '0', value: '0' }, { label: '1–2', value: '1-2' }, { label: '3–5', value: '3-5' }, { label: '5+', value: '5+' }]}
        />
      </div>
      ...
      <button
        ...
        className={`w-full rounded-lg px-4 py-3 text-sm font-medium
          ${allAnswered ? 'bg-gray-900 text-white hover:bg-black' : 'cursor-not-allowed bg-gray-100 text-gray-400'}`}
      >
        Continue
      </button>
    </div>
...
function CancelYesStep2({
  cancellationData, patchCancellation,
  feedback, setFeedback, minChars, onContinue,
}: {
  cancellationData: CancellationRow; patchCancellation: (p: Partial<CancellationRow>) => void;
  feedback: string; setFeedback: (v: string) => void; minChars: number;
  onContinue: () => void;
}) {
    ...
    <div>
      ...
      <div className="relative">
        <textarea
          ...
          onChange={(e) => {
            ...
            patchCancellation({ feedback: e.target.value })
          }}
          rows={6}
      ...
      <button
        ...
        className={`mt-4 w-full rounded-lg px-4 py-3 text-sm font-medium
          ${canContinue ? 'bg-gray-900 text-white hover:bg-black' : 'cursor-not-allowed bg-gray-100 text-gray-400'}`}
      >
        Continue
      </button>
    </div>
...
function VisaQuestionStep3({
      ...
      <label className="flex items-center gap-2 cursor-pointer select-none text-sm text-gray-800">
        ...
        <span
          className={`flex h-4 w-4 items-center justify-center rounded-full border
            ${checked ? 'border-black-600 ring-4 ring-emerald-100' : 'border-gray-400'}`}
          aria-hidden
        >
  ...
  const heading = viaMM ? (
    <>
      We helped you land the job, now<br className="hidden sm:block" />
      let’s help you secure your visa.
    </>
  ) : (
    <>
      <span className="block">You landed the job!</span>
      <span className="block italic font-semibold">That’s what we live for.</span>
    </>
  );
    ...
    <div>
      ...
      <button
        ...
        className={`mt-1 w-full rounded-lg px-4 py-3 text-sm font-medium
          ${canSubmit ? 'bg-gray-900 text-white hover:bg-black'
...
async function fetchSubscriptionStub(_email?: string): Promise<{ monthlyPrice: number; downsellVariant?: DownsellVariant }> {
  await new Promise(r => setTimeout(r, 150));
  return { monthlyPrice: 25, downsellVariant: undefined };
}

async function stubGetOrCreateDownsellVariant(userEmail?: string): Promise<DownsellVariant> {
  const key = `mm_downsell_variant:${userEmail ?? 'anon'}`;
  if (typeof window === 'undefined') return pickDownsellVariant();
  const saved = window.localStorage.getItem(key);
  if (saved === 'A' || saved === 'B') return saved as DownsellVariant;
  const v = pickDownsellVariant();
  try { window.localStorage.setItem(key, v); } catch {}
  return v;
}

async function stubGetExistingCancellationRow(_userId: string): Promise<CancellationRow | null> {
  await new Promise(r => setTimeout(r, 120));
  //return null; // return a row here to simulate hydration
  ...
  //   // Usage (already answered)
  //   applied_count: '6-20',
  //   emailed_count: '1-5',
  //   interview_count: '1-2',

  //   // Reason not chosen yet (you’ll be on the reason page next)
  //   reason: 'Too expensive',
  //   reason_text: '20',

  //   // YES-flow (not applicable/empty yet)
  ...
  // YES flow: user found a job, attributed to MM, usage filled, visa details filled
  return {
    id: 14,
    user_id: _userId,
    subscription_id: 12,

    // Not applicable in YES flow
    ...
    // Reason fields not used in YES flow
    reason: null,
    reason_text: null,
    feedback:'helloooooooooooooooooooooooo',

    // YES-flow specifics
    attributed_to_mm: false,     // user says MM helped land the job
    visa_has_lawyer: false,     // company not providing a lawyer
    visa_type: 'H-1B',          // user-entered visa type
...
async function stubPersistSubscriptionAndCancellation(input: {
  user: User;
  subscription: Subscription;
  cancellation: CancellationRow | null;
}) {
  console.log('stubPersistSubscriptionAndCancellation', input);
...
function NoDownsellFunction({
  ...
  const [monthlyPrice, setMonthlyPrice] = useState<number>(25);

  useEffect(() => {
    let alive = true;
    (async () => {
      setLoading(true);
      const sub = await fetchSubscriptionStub(userEmail);
      if (!alive) return;
  ...
  const discountedPrice = Math.max(0, monthlyPrice - 10);
  ...
  const fmt = (n: number) => `$${Number.isInteger(n) ? n.toFixed(0) : n.toFixed(2)}/month`;
  ...
  return (
    <div>
      ...
      <p className="mt-2 text-gray-600">We’ve been there and we’re here to help you.</p>

      <div className="mt-5 rounded-2xl border border-violet-200 bg-violet-50 p-4 text-center">
        <p className="text-lg font-semibold text-gray-900">
          Here’s <span className="underline">{offer.discountLabel}</span> until you find a job.
        </p>

        <div className="mt-2 flex items-baseline justify-center gap-3">
          <span className="text-2xl font-bold text-indigo-700">{fmt(offer.discountedPrice)}</span>
          <span className="text-sm text-gray-500 line-through">{fmt(offer.monthlyPrice)}</span>
        </div>
        ...
        <p className="mt-2 text-xs text-gray-500">You won’t be charged until your next billing date.</p>
      </div>
...
async function stubSaveNoFlowSurvey(data: {
  offer?: { variant: DownsellVariant; monthlyPrice: number };
  usage: { applied?: string; emailed?: string; interviews?: string };
  reason: { main?: string; other?: string; rating?: number };
}) {
  console.log('stubSaveNoFlowSurvey', data);
  await new Promise(r => setTimeout(r, 150));
}
...
function NoDownsellAccepted({
  ...
  const dateStr = nextBillingDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric' });

  return (
    <div className="md:order-1">
      <h2 className="text-2xl sm:text-[26px] font-extrabold text-gray-900">Great choice, mate!</h2>
      ...
      <p className="mt-4 text-sm text-gray-600">
        ...
        <br />
        Starting from {dateStr}, your monthly payment will be ${discountedPrice.toFixed(2)}.
      </p>
      <p className="mt-2 text-xs text-gray-400">You can cancel anytime before then.</p>
...
function OfferDeclinedUsageStep({
  ...
  emailedCount, setEmailedCount,
  interviewsCount, setInterviewsCount,
  onBack, onContinue, onAcceptOffer, onClose,
}: {
  cancellationData: CancellationRow; patchCancellation: (p: Partial<CancellationRow>) => void;
  offer?: DownsellOffer;
  appliedCount?: string; setAppliedCount: (v: string) => void;
  ...
  return (
    <div>
      <h2 className="text-[22px] sm:text-[24px] font-bold leading-tight text-gray-900">
        Help us understand how you<br className="hidden sm:block" />
      ...
      {offer && (
        <button
          ...
          onClick={(e) => { e.preventDefault(); e.stopPropagation(); onAcceptOffer(); }}
        ...
        >
          <span className="flex flex-wrap items-center justify-center gap-2 sm:gap-3 text-sm sm:text-base font-normal">
            ...
            <span aria-hidden className="hidden sm:inline h-4 w-px bg-white/40" />
            <span className="tabular-nums">
              {fmt(offer.discountedPrice)}<span className="ml-0.5">/mo</span>
            </span>
      ...
      <button
        ...
        className={`mt-3 w-full rounded-lg px-4 py-3 text-sm font-medium
          ${allAnswered ? 'bg-red-600 text-white hover:bg-red-700'
                        : 'cursor-not-allowed bg-gray-100 text-gray-400'}`}
      >
        Continue
      </button>
...
function NoMainReasonStep3({
  ...
  rating, setRating,
  onBack, onClose, onAcceptOffer, onComplete,
}: {
  cancellationData: CancellationRow; patchCancellation: (p: Partial<CancellationRow>) => void;
  offer?: DownsellOffer;
  reason?: string; setReason: (v: string) => void;
  otherText: string | any; setOtherText: (v: string | any) => void;
  rating?: number; setRating: (n?: number) => void;
  onBack: () => void; onClose: () => void;
  onAcceptOffer: () => void; onComplete: () => void;
}) {
  ...
  const prompts: Record<typeof REASONS[number], string> = {
    ...
    'Not enough relevant jobs': 'In what way can we make the jobs more relevant?*',
  ...
  const priceOK =
    !isExpensive || (maxPrice.trim() !== '' && !Number.isNaN(Number(maxPrice)) && Number(maxPrice) >= 0);

  const chars = otherText.length;
  const feedbackOK = isExpensive || chars >= minChars;

  const canComplete = Boolean((reason && priceOK && feedbackOK) || (reason && otherText && feedbackOK))
  ...
  const RadioRow = ({ label }: { label: (typeof REASONS)[number] }) => (
    <label className="flex items-center gap-2 cursor-pointer select-none py-2 text-sm text-gray-800">
      ...
      <span
        aria-hidden
        className={`flex h-4 w-4 items-center justify-center rounded-full border ${
          reason === label ? 'border-gray-900 ring-4 ring-gray-200' : 'border-gray-400'
        }`}
      >
        <span className={`h-2 w-2 rounded-full ${reason === label ? 'bg-gray-900' : 'bg-transparent'}`} />
      </span>
      {label}
    </label>
  );
  ...
  return (
    <div>
      ...
      <p className="text-xs text-gray-500 mb-3">Please take a minute to let us know why:</p>

      {!reason ? (
        <div className="space-y-1 mb-2">
          {REASONS.map((r) => <RadioRow key={r} label={r} />)}
        </div>
      ) : (
        <div className="mb-3">
            ...
            <div className="mt-2">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onBlur={() => setOtherText((v: string|any) => v.replace(/\.$/, ''))}
      ...
      <button
        ...
        onClick={onComplete}
        className={`mt-3 w-full rounded-lg px-4 py-3 text-sm font-medium ${
          canComplete ? 'bg-gray-900 text-white hover:bg-black'
                      : 'cursor-not-allowed bg-gray-100 text-gray-400'
        }`}
      >
        Complete cancellation
  ...
  return (
    <div>
      ...
      <p className="mt-3 text-xl font-semibold text-gray-900">
        Thanks for being with us, and you’re<br className="hidden sm:block" />
        always welcome back.
      </p>
...
export default function CancelComponent({ isOpen, onClose, userEmail, userDetails, subscriptionDetails }: Props) {
  ...
  const [declinedOffer, setDeclinedOffer] = useState<DownsellOffer | undefined>(undefined);
  ...
  const [subscriptionData, setSubscriptionData] = useState<Subscription>(subscriptionDetails);
  ...
  const patchCancellation = (patch: Partial<CancellationRow>) =>
    setCancellationData(prev => ({ ...prev, ...patch, updated_at: new Date().toISOString() }));
  ...
  const noStepActive = React.useCallback(
    (r: Route): number | undefined => {
      ...
      switch (r) {
        case 'no.step1': return isNoDownsell ? undefined : 1;
        case 'no.usage': return isNoDownsell ? 1 : 2;
        case 'no.step3': return isNoDownsell ? 2 : 3;
  ...
  // hydrate any existing cancellation row
  useEffect(() => {
    if (!isOpen) return;
    let alive = true;
    (async () => {
      const row = await stubGetExistingCancellationRow(userDetails.id);
      if (!alive || !row) {
  ...
  const goNoFlow = async () => {
    const v = abVariant ?? await stubGetOrCreateDownsellVariant(userEmail);
    setAbVariant(v);
  ...
  const handlePersistAndClose = async () => {
    try {
      console.log("Persisting Data:", {
      subscription: subscriptionData,
      cancellation: cancellationData,
    });
      await stubPersistSubscriptionAndCancellation({
        user: userDetails,
        subscription: subscriptionData,
        cancellation: cancellationData,
      });
  ...
  return (
    ...
    >
      ...
      >
        {/* Step: Question */}
        {route === 'question' && (
          <SectionFrame imageMode="top-mobile-right-desktop" onClose={handlePersistAndClose}>
            <CancelQuestion
              onYes={() => {
                setSubscriptionData((prev) => ({ ...prev, status: 'pending_cancellation' }));
                setRoute('yes.step1');
              }}
              onNo={() => {
                setSubscriptionData((prev) => ({ ...prev, status: 'pending_cancellation' }));
                const v = abVariant!= null ? abVariant : getVariantStable(userEmail);
                setAbVariant(v);
                patchCancellation({ downsell_variant: v });
        ...
        {/* YES – 1 */}
        {route === 'yes.step1' && (
          <SectionFrame showBack onBack={() => setRoute('question')} stepActive={1} stepTotal={3} imageMode="right-hide-mobile" onClose={handlePersistAndClose}>
            <CancelYesStep1
              cancellationData={cancellationData}
              patchCancellation={patchCancellation}
              userEmail={userEmail}
              foundViaMM={foundViaMM}
              setFoundViaMM={(v) => { setFoundViaMM(v); patchCancellation({ attributed_to_mm: v === 'yes' }); }}
              appliedCount={appliedCount}
              setAppliedCount={(v) => { setAppliedCount(v); patchCancellation({ applied_count: v as AppliedCount }); }}
              emailedCount={emailedCount}
              setEmailedCount={(v) => { setEmailedCount(v); patchCancellation({ emailed_count: v as EmailedCount }); }}
              interviewsCount={interviewsCount}
              setInterviewsCount={(v) => { setInterviewsCount(v); patchCancellation({ interview_count: v as InterviewCount }); }}
        ...
        {/* YES – 2 */}
        {route === 'yes.step2' && (
          <SectionFrame showBack onBack={() => setRoute('yes.step1')} stepActive={2} stepTotal={3} imageMode="right-hide-mobile" onClose={handlePersistAndClose}>
            <CancelYesStep2
              cancellationData={cancellationData}
              patchCancellation={patchCancellation}
              feedback={feedback}
              setFeedback={(t) => { setFeedback(t); }}
              minChars={minChars}
        ...
        {/* YES – 3 */}
        {route === 'yes.step3' && (
          <SectionFrame showBack onBack={() => setRoute('yes.step2')} stepActive={3} stepTotal={3} imageMode="right-hide-mobile" onClose={handlePersistAndClose}>
            <VisaQuestionStep3
              cancellationData={cancellationData}
              patchCancellation={patchCancellation}
              foundViaMM={foundViaMM}
              visaLawyer={visaLawyer}
              setVisaLawyer={(v) => { setVisaLawyer(v); patchCancellation({ visa_has_lawyer: v === 'yes' }); }}
              visaType={visaType}
              setVisaType={(v) => { setVisaType(v); patchCancellation({ visa_type: v }); }}
        ...
        {/* YES – Success */}
        {route === 'yes.done' && (
          <SectionFrame title="Subscription Cancelled" showBack onBack={() => setRoute('yes.step3')} imageMode="right-hide-mobile" onClose={handlePersistAndClose}>
            <CancelSuccess
              visaLawyer={visaLawyer}
              contactName="Mihailo Basic"
              contactEmail="mihailo@migratemate.co"
          ...
          >
            <NoDownsellFunction
              ...
              onAccept={async (offer) => {
                patchCancellation({ accepted_downsell: true, downsell_variant: offer.variant });
               setSubscriptionData(prev => ({
                ...
                const nextBillingDate = new Date(subscriptionData.currentPeriodEnd);
                const daysLeft = Math.max(0, Math.ceil((nextBillingDate.getTime() - Date.now()) / (24*60*60*1000)));
                setDownsellAccepted({
        ...
        {route === 'no.step2' && downsellAccepted && (
          <SectionFrame title="Subscription" mobileTitle="Subscription Continued" imageMode="top-mobile-right-desktop" onClose={handlePersistAndClose}>
            <NoDownsellAccepted
        ...
        {route === 'no.usage' && (
          ...
          >
            <OfferDeclinedUsageStep
              ...
              patchCancellation={patchCancellation}
              offer={declinedOffer}
              appliedCount={appliedCount} setAppliedCount={(v) => { setAppliedCount(v); patchCancellation({ applied_count: v as AppliedCount }); }}
              emailedCount={emailedCount} setEmailedCount={(v) => { setEmailedCount(v); patchCancellation({ emailed_count: v as EmailedCount }); }}
              interviewsCount={interviewsCount} setInterviewsCount={(v) => { setInterviewsCount(v); patchCancellation({ interview_count: v as InterviewCount }); }}
              onBack={() => setRoute('no.step1')}
              onClose={handlePersistAndClose}
              onAcceptOffer={async () => {
                if (!declinedOffer) return;
                patchCancellation({ accepted_downsell: true, downsell_variant: declinedOffer.variant });
                setSubscriptionData(prev => ({ ...prev, status: 'active', monthlyPrice: declinedOffer.discountedPrice }));
                const nextBillingDate = new Date(subscriptionData.currentPeriodEnd);
                const daysLeft = Math.max(0, Math.ceil((nextBillingDate.getTime() - Date.now()) / (24*60*60*1000)));
                setDownsellAccepted({
        ...
        {/* NO – reason */}
        {route === 'no.step3' && (
          <SectionFrame showBack onBack={() => setRoute('no.usage')} stepActive={isNoDownsell ? 2 : 3} stepTotal={noTotalSteps} imageMode="right-hide-mobile" onClose={handlePersistAndClose}>
            <NoMainReasonStep3
              cancellationData={cancellationData}
              patchCancellation={patchCancellation}
              ...
              setReason={(v) => { 
                setCancelReason(v); 
                patchCancellation({ reason: v === 'Other' ? (cancelOther ? `Other: ${cancelOther}` : 'Other') : v });
              }}
              otherText={cancelOther}
              setOtherText={(t) => {
                setCancelOther(t);
                patchCancellation({ reason: cancelReason === 'Other' ? `Other: ${t}` : (cancelReason ?? null) });
              }}
              rating={satisfaction}
              ...
              onComplete={async () => {
                patchCancellation({ reason: cancelReason, reason_text: cancelOther  });
                await stubSaveNoFlowSurvey({
                  offer: declinedOffer ? { variant: declinedOffer.variant, monthlyPrice: declinedOffer.monthlyPrice } : undefined,
                  usage: { applied: appliedCount, emailed: emailedCount, interviews: interviewsCount },
                  reason: { main: cancelReason, other: cancelOther, rating: satisfaction },
                });
                setRoute('no.done');
              }}
            />
        ...
        {/* NO – done */}
        {route === 'no.done' && (
          <SectionFrame title="Subscription Cancelled" imageMode="right-hide-mobile" onClose={handlePersistAndClose}>
